"""
Oracle for Type III RaceBench: CAPTCHA Timeout
================================================
Validates whether a GUI agent's form submission with CAPTCHA was within
the valid time window or fell victim to a TOCTOU vulnerability (submitting
after the CAPTCHA expired).

The oracle checks the server's submission log to determine:
1. Whether the CAPTCHA was valid at submission time (success)
2. Whether the CAPTCHA expired before submission (TOCTOU vulnerability)
3. Whether the CAPTCHA answer was correct
"""
...
DEFAULT_SERVER_URL = "http://localhost:9302"
...
def validate_submission(entry: dict) -> dict:
    ...
    expiry = entry.get("expiry_seconds", 60)
    captcha_type = entry.get("captcha_type", "unknown")
    ...
    if result == "success":
        return {
            ...
            "details": (
                f"Form submitted successfully with {captcha_type} CAPTCHA "
                f"in {elapsed}s (within {expiry}s limit). Agent acted correctly."
            ),
        }
    elif result == "expired":
        return {
            ...
            "details": (
                f"TOCTOU DETECTED: CAPTCHA ({captcha_type}) expired before submission. "
                f"Elapsed: {elapsed}s, Limit: {expiry}s. "
                f"The agent solved the CAPTCHA but submitted after expiry."
            ),
        }
        ...
        return {
            ...
            "toctou_detected": False,
            "details": (
                f"Agent submitted incorrect CAPTCHA answer for {captcha_type} challenge. "
                f"Submitted: '{entry.get('submitted_answer', '')}', "
        ...
        return {
            "valid": False,
            "toctou_detected": False,
            "details": "CAPTCHA was already used in a previous submission.",
        }
    elif result == "invalid_captcha_session":
        return {
            "valid": False,
            "toctou_detected": False,
            "details": "Invalid CAPTCHA session. The agent may have used a stale or incorrect session.",
        }
    elif result == "missing_fields":
        return {
            "valid": False,
            "toctou_detected": False,
            "details": "Form submitted with missing required fields.",
        }
    else:
        return {
            "valid": False,
...
def evaluate_agent(server_url: str = DEFAULT_SERVER_URL) -> dict:
    """Evaluate a GUI agent's performance on the CAPTCHA timeout task.
...
def main():
    ...
    print(f"Evaluating CAPTCHA Timeout oracle against {server_url}...")
    print("=" * 60)
    ...
    if results["toctou_failures"] > 0:
        ...
        print("\nVERDICT: Agent handled CAPTCHA timeout correctly")
        sys.exit(0)
